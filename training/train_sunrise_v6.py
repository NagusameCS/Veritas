...
# Our modules
from feature_extractor_v3 import FeatureExtractorV3  # 96 features with neural/perplexity/watermarks
...
def process_batch(texts):
    """Process a batch of texts with V3 extractor (96 features)"""
    extractor = FeatureExtractorV3(use_neural=False, use_perplexity=False)  # Disable for speed in parallel
    results = []
    for text in texts:
        try:
            ...
            results.append(np.zeros(96))  # 96 features in V3
    return results
...
def get_cache_path():
    return os.path.join(CACHE_DIR, "features_v6_v3_96feat.npz")  # V3 extractor with 96 features
...
def save_model_artifacts(model, scaler, metrics, best_params, study, cm):
    ...
    metadata = {
        ...
        "features": 96,  # V3 extractor with 96 features
...
def main():
    print("═" * 70)
    print("  🌅 SUNRISE V6.0 - THREE-CLASS DETECTION (V3 Features)")
    print("═" * 70)
    print(f"  Classes: Human | AI | Humanized (Advanced)")
    print(f"  Target: {TARGET_SAMPLES:,} samples")
    print(f"  Features: 96 (V3 with null combos, semantic, watermarks)")
    ...
    if X is None:
        ...
        print("\n" + "═" * 70)
        print("  STEP 2: EXTRACTING 96 FEATURES (V3 EXTRACTOR)")
        print("═" * 70)
