#!/usr/bin/env python3
"""
SUPERNOVA ULTRA - RAID Benchmark with Random Sampling
Tests on randomly sampled data (different from training samples)
"""
...
print("=" * 70)
print("SUPERNOVA ULTRA - RAID BENCHMARK (Random Sampling)")
print("=" * 70)
...
# Load RAID with random sampling (different from training)
print("\n📊 Loading RAID test data (random sampling for benchmark)...")
dataset = load_dataset("liamdugan/raid", split="train", streaming=True)

# We'll sample randomly from a different region than training
# Training sampled from positions 0 to 5.6M sparsely
# We'll sample from the middle and end regions
...
# Use a random seed different from training
random.seed(9999)

for row in tqdm(dataset, desc="Scanning RAID"):
    row_counter += 1
    
    # Skip early samples where training sampled heavily
    if row_counter < 100000:
        continue
        
    # Random sampling - only take ~2% of remaining data
    ...
    if all(c >= SAMPLES_PER_ATTACK for c in attack_counts.values()) and len(attack_counts) >= 10:
        print(f"\n  ✓ Collected sufficient samples at row {row_counter:,}")
        break
    
    if len(samples) >= MAX_SAMPLES:
        print(f"\n  ✓ Reached max samples at row {row_counter:,}")
        break
...
print("\n" + "=" * 70)
print("SUPERNOVA ULTRA - RAID BENCHMARK RESULTS")
print("=" * 70)
print(f"\n  Test samples: {len(y)} (randomly sampled)")
