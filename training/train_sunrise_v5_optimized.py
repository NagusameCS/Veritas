#!/usr/bin/env python3
"""
Sunrise V5.0 - Million Sample Training Pipeline (Optimized)
============================================================
- 1,000,000+ samples from HuggingFace datasets (parquet-based only)
- Full provenance tracking for every sample
- Maximum accuracy optimization with 100 trials
...
N_WORKERS = 15
OPTUNA_TRIALS = 100  # Maximum optimization
BATCH_SIZE = 50000

# Only parquet-based datasets that work reliably (no script-based loaders)
DATASETS_CONFIG = [
    # === HUMAN TEXT SOURCES (Large scale) ===
    # News - Large parquet datasets
    ...
    # Large text corpora - using streaming-friendly versions
    ...
    # ChatGPT comparison datasets
    {"name": "Hello-SimpleAI/HC3", "config": "all", "split": "train", "text_field": "chatgpt_answers", "label": 1, "max_samples": 100000, "source_type": "chatgpt"},
    
    # AI detection datasets with labels
    ...
    {"name": "Open-Orca/OpenOrca", "config": None, "split": "train", "text_field": "response", "label": 1, "max_samples": 100000, "source_type": "llm_response"},
    
    # More AI sources
    {"name": "HuggingFaceH4/ultrachat_200k", "config": None, "split": "train_sft", "text_field": "messages", "label": 1, "max_samples": 80000, "source_type": "ultrachat"},
    {"name": "databricks/databricks-dolly-15k", "config": None, "split": "train", "text_field": "response", "label": 1, "max_samples": 15000, "source_type": "dolly"},
    {"name": "tatsu-lab/alpaca", "config": None, "split": "train", "text_field": "output", "label": 1, "max_samples": 52000, "source_type": "alpaca"},
    
    # Human writing samples
    {"name": "Anthropic/hh-rlhf", "config": None, "split": "train", "text_field": "chosen", "label": 0, "max_samples": 80000, "source_type": "human_feedback"},
]
...
class SampleReceipt:
    """Tracks provenance for each sample with full metadata"""
    ...
    def add_sample(self, sample_data):
        """Add a sample receipt with full provenance"""
        receipt = {
    ...
    def _save_batch(self):
        ...
        self.total_saved += len(self.receipts)
        print(f"    💾 Saved {len(self.receipts):,} receipts (batch {self.batch_count}, total: {self.total_saved:,})")
        self.batch_count += 1
        self.receipts = []
    ...
    def finalize(self):
        ...
        return self.batch_count, self.total_saved
...
def extract_text_from_item(item, text_field, dataset_name):
    ...
    try:
        ...
        # Handle list fields (conversations, messages, etc.)
                ...
                if isinstance(value[0], str):
                    ...
                    for v in value:
                        if isinstance(v, dict):
                            ...
                            content = v.get('content', v.get('value', v.get('text', '')))
                            if content:
...
def load_dataset_samples(config, receipt_tracker, collected_hashes):
    ...
    try:
        ...
        # Load dataset (no trust_remote_code to avoid script-based loaders)
        if dataset_config:
        ...
        print(f"    📊 Available: {total_available:,}, sampling: {n_to_load:,}")
...
def collect_all_samples(target_count, receipt_tracker):
    ...
    print(f"  Max possible: {total_configured:,}\n")
    
    for config in DATASETS_CONFIG:
        if len(all_samples) >= target_count * 1.1:
            break
            
        samples = load_dataset_samples(config, receipt_tracker, collected_hashes)
...
# ══════════════════════════════════════════════════════════════════════════════
# HYPERPARAMETER OPTIMIZATION (MAXIMUM)
# ══════════════════════════════════════════════════════════════════════════════

def create_optuna_objective(X_train, y_train, X_val, y_val):
    """Create Optuna objective for maximum optimization"""
...
def optimize_hyperparameters(X_train, y_train, n_trials=100):
    ...
    # Create study with multiple samplers for better exploration
...
def train_final_model(X_train, y_train, best_params):
    ...
    if model_type == "rf":
        valid_params = ["n_estimators", "max_depth", "min_samples_split", "min_samples_leaf", 
                       "max_features", "class_weight", "bootstrap", "criterion", "n_jobs", "random_state"]
        params = {k: v for k, v in best_params.items() if k in valid_params}
        params["n_jobs"] = -1
        ...
        valid_params = ["n_estimators", "max_depth", "learning_rate", "min_samples_split", 
...
def save_model(model, scaler, metrics, best_params, feature_importance, 
    ...
    receipt = {
        ...
        "best_optimization_accuracy": study.best_value,
    ...
    print(f"  ✓ Saved: optuna_trials.json ({len(trials_data)} trials)")
...
const VERITAS_MODEL_CONFIG = {{
    ...
    humanSamples: {training_stats['human_samples']},
    aiSamples: {training_stats['ai_samples']},
    optimizationTrials: {OPTUNA_TRIALS},
    bestModelType: "{best_params.get('model_type', 'rf')}",
    trained: "{datetime.now().isoformat()}",
    receiptsLocation: "{RECEIPTS_DIR}",
...
def main():
    ...
    # Finalize receipts
    batch_count, total_receipts = receipt_tracker.finalize()
    print(f"\n  📝 Saved {batch_count} receipt batches ({total_receipts:,} samples) to {RECEIPTS_DIR}")
